...
class ArmWasdInterface(object):
    ...
    def get_arm_pose(self):
        ...
        position_array = np.array([
            odom_T_flat_body.position.x,
            odom_T_flat_body.position.y,
            odom_T_flat_body.position.z
        ])
        euler_array = self.quat_2_euler(odom_T_flat_body.rotation)
        self.arm_pose = np.concatenate((position_array, euler_array))

    def quat_2_euler(self, quat):
        # Convert to Euler angles (in degrees)
        euler_angle = Rotation.from_quat([quat.x, quat.y, quat.z, quat.w]).as_euler('xyz', degrees=True)
        return euler_angle
    
    def euler_2_quat(self, euler):
        # Convert to Euler angles (in degrees)

...
def stitch(robot, options):
    """Stitch two front fisheye images together and save the result as JPG"""
    pygame.init()
    ...
    while running:
        # pygame no need when actual running. 
        pygame.display.set_mode(display, pygame.DOUBLEBUF | pygame.OPENGL)
        ...
        if images_future is not None and images_future.done():
            try:
                ...
                if front_right is not None and front_left is not None:
                    ...
                    image = Image.frombytes("RGB", display, pixels) 
                    image = image.transpose(Image.FLIP_TOP_BOTTOM)  # Flip image to correct orientation

...
def create_episode_from_files(path, prompt, img_path, hand_path, state_path, action_path):
    ...
    for step in range(episode_length):
        ...
        episode.append({
            ...
            'state': state,
            'action': action,
            'language_instruction': prompt,  # Folder name as language instruction
        })
...
train_folder_paths = [os.path.join(root_data_path, folder) for folder in os.listdir(root_data_path) if os.path.isdir(os.path.join(root_data_path, folder))]

# Iterate over each folder to create train episodes
os.makedirs('data/train', exist_ok=True)
os.makedirs('data/val', exist_ok=True)

print(train_folder_paths)
for j, task_path in enumerate(tqdm.tqdm(train_folder_paths)):
    episode_paths = [os.path.join(task_path, folder) for folder in os.listdir(task_path) if os.path.isdir(os.path.join(task_path, folder))]
    for i, folder_path in enumerate(episode_paths):
               
        # Define paths to the .avi and .npz files within each folder
        img_file_path = os.path.join(folder_path, 'rgb_video.avi')
        hand_file_path = os.path.join(folder_path, 'rgb_hand_video.avi')
        state_file_path = os.path.join(folder_path, 'actual_poses.npz')
        action_file_path = os.path.join(folder_path, 'target_poses.npz')

        folder_name = os.path.basename(folder_path)
        # Use regex to find content inside curly braces
        match = re.search(r'\{(.*?)\}', folder_name)
        if match:
            prompt = match.group(1) 
        
            # zyb! attention, the folder name contains [space], and used for filename. 
            if i % TrainVal_div == 0:
                create_episode_from_files(f'data/val/{prompt}_{j}_{i}.npy', prompt, img_file_path, hand_file_path, state_file_path, action_file_path) 
            else:
                create_episode_from_files(f'data/train/{prompt}_{j}_{i}.npy', prompt, img_file_path, hand_file_path, state_file_path, action_file_path) 

        else:
            prompt = 'move the items in sequence' # The content inside the braces
            create_episode_from_files(f'data/sequence/{prompt}_{j}_{i}.npy', prompt, img_file_path, hand_file_path, state_file_path, action_file_path) 


print("Generating val examples...")
# Use the same logic for validation if needed, here assuming 5 validation episodes

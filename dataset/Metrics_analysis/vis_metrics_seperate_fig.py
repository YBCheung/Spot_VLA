...
def create_multi_metric_visualizations(intra_task_metrics, inter_task_metrics, instruction_to_actions):
    """
    Create separate comprehensive visualizations for each metric (DTW, OT, and Cosine Similarity)
    """
    ...
    for metric, metric_name in zip(metrics, metric_names):
        # Figure: Individual metric analysis (2x2 grid)
        plt.figure(figsize=(16, 12))
        
        # Subplot 1: Intra-task vs Inter-task comparison
        plt.subplot(2, 2, 1)
        ...
        plt.title(f'{metric_name}: Intra vs Inter-task Distribution')
        if metric in ['dtw', 'ot']:
            plt.yscale('log')
        
        # Add statistical annotation
        from scipy.stats import mannwhitneyu
        stat, p_value = mannwhitneyu(intra_data, inter_data, alternative='two-sided')
        plt.text(0.5, 0.95, f'Mann-Whitney U p-value: {p_value:.2e}', 
                transform=plt.gca().transAxes, ha='center',
                bbox=dict(boxstyle="round", facecolor='lightblue', alpha=0.7))
        
        # Subplot 2: Task similarity heatmap
        plt.subplot(2, 2, 2)
        ...
        task_labels = [t[:15] + '...' if len(t) > 15 else t for t in tasks]
        
        sns.heatmap(metric_matrix, 
                    xticklabels=task_labels, 
                    yticklabels=task_labels, 
                    annot=True, fmt='.3f', 
                    cmap='viridis' if metric != 'cosine' else 'RdYlBu_r',
                    cbar_kws={'label': metric_name})
        plt.title(f'{metric_name} Task Similarity Matrix')
        plt.xticks(rotation=45, ha='right')
        plt.yticks(rotation=0)
        
        # Subplot 3: Distribution per task
        plt.subplot(2, 2, 3)
        task_data = []
        task_labels = []
        ...
        for task, task_metrics in intra_task_metrics.items():
            ...
            task_labels.extend([task[:20] + '...' if len(task) > 20 else task] * len(task_metrics[metric]))
        
        df_task = pd.DataFrame({metric_name: task_data, 'Task': task_labels})
        
        # Sort tasks by median value for better visualization
        task_medians = df_task.groupby('Task')[metric_name].median().sort_values()
        df_task['Task'] = pd.Categorical(df_task['Task'], categories=task_medians.index, ordered=True)
        
        sns.boxplot(data=df_task, y='Task', x=metric_name, orient='h')
        plt.title(f'{metric_name} Distribution by Task (Intra-task)')
        if metric in ['dtw', 'ot']:
            plt.xscale('log')
        
        # Subplot 4: Task separability scores
        plt.subplot(2, 2, 4)
        separability_scores = []
        task_names = []
        ...
        for task in intra_task_metrics.keys():
            ...
            if inter_values:
                ...
                if metric == 'cosine':
                    # For cosine similarity, higher intra-task similarity is better
                    separability = intra_mean / inter_mean if inter_mean > 0 else 0
                else:
                    # For DTW and OT, lower intra-task distance is better
        ...
        # Sort by separability score
        sorted_pairs = sorted(zip(separability_scores, task_names), reverse=True)
        separability_scores, task_names = zip(*sorted_pairs)
        
        plt.barh(range(len(task_names)), separability_scores, 
                color='green' if metric == 'cosine' else 'blue', alpha=0.7)
        plt.yticks(range(len(task_names)), task_names)
        plt.xlabel(f'Separability Ratio ({metric_name})')
        plt.title(f'{metric_name} Task Separability Scores')
        plt.grid(axis='x', alpha=0.3)
        
        # Add separability interpretation
        avg_sep = np.mean(separability_scores)
        plt.axvline(avg_sep, color='red', linestyle='--', alpha=0.7, 
                   label=f'Average: {avg_sep:.2f}')
        plt.legend()
        
        plt.tight_layout()
        plt.savefig(f'{metric}_analysis_detailed.png', dpi=300, bbox_inches='tight')
        plt.close()
        
        print(f"Saved detailed analysis for {metric_name}: {metric}_analysis_detailed.png")
...
def create_metric_correlation_analysis(intra_task_metrics, inter_task_metrics):
    """
    Create detailed correlation analysis between all three metrics
    """
    plt.figure(figsize=(16, 12))
    ...
    # Combine intra and inter data
    all_dtw = all_dtw_intra + all_dtw_inter
    all_ot = all_ot_intra + all_ot_inter
    all_cos = all_cos_intra + all_cos_inter
    
    # Create color coding for intra vs inter
    colors = ['red'] * len(all_dtw_intra) + ['blue'] * len(all_dtw_inter)
    
    # Subplot 1: DTW vs OT
    plt.subplot(2, 3, 1)
    plt.scatter(all_dtw, all_ot, c=colors, alpha=0.6, s=20)
    ...
    # Add trend line
    z = np.polyfit(all_dtw, all_ot, 1)
    p = np.poly1d(z)
    plt.plot(all_dtw, p(all_dtw), "k--", alpha=0.8, linewidth=1)
    
    # Subplot 2: DTW vs Cosine
    plt.subplot(2, 3, 2)
    plt.scatter(all_dtw, all_cos, c=colors, alpha=0.6, s=20)
    plt.xlabel('DTW Distance')
    ...
    # Add trend line
    z = np.polyfit(all_dtw, all_cos, 1)
    p = np.poly1d(z)
    plt.plot(all_dtw, p(all_dtw), "k--", alpha=0.8, linewidth=1)
    
    # Subplot 3: OT vs Cosine
    plt.subplot(2, 3, 3)
    plt.scatter(all_ot, all_cos, c=colors, alpha=0.6, s=20)
    ...
    # Add trend line
    z = np.polyfit(all_ot, all_cos, 1)
    p = np.poly1d(z)
    plt.plot(all_ot, p(all_ot), "k--", alpha=0.8, linewidth=1)
    
    # Subplot 4: 3D scatter plot
    ax = plt.subplot(2, 3, 4, projection='3d')
    scatter = ax.scatter(all_dtw, all_ot, all_cos, c=colors, alpha=0.6, s=20)
    ...
    red_patch = plt.Line2D([0], [0], marker='o', color='w', markerfacecolor='red', markersize=8, label='Intra-task')
    blue_patch = plt.Line2D([0], [0], marker='o', color='w', markerfacecolor='blue', markersize=8, label='Inter-task')
    ax.legend(handles=[red_patch, blue_patch], loc='upper right')
    
    # Subplot 5: Metric agreement analysis
    plt.subplot(2, 3, 5)
    ...
    agreement_stds = [np.std(agreement_dtw_ot), np.std(agreement_dtw_cos), np.std(agreement_ot_cos)]
    agreement_labels = ['DTW-OT', 'DTW-Cosine', 'OT-Cosine']
    
    bars = plt.bar(agreement_labels, agreements, yerr=agreement_stds, 
                   capsize=5, alpha=0.7, color=['purple', 'orange', 'green'])
    ...
    correlation_matrix = np.array([
        [1.0, corr_dtw_ot, corr_dtw_cos],
        [corr_dtw_ot, 1.0, corr_ot_cos],
        [corr_dtw_cos, corr_ot_cos, 1.0]
    ])
    
    sns.heatmap(correlation_matrix, 
                xticklabels=['DTW', 'OT', 'Cosine'],
                yticklabels=['DTW', 'OT', 'Cosine'],
                annot=True, fmt='.3f', cmap='RdBu_r', center=0,
                square=True, cbar_kws={'label': 'Correlation Coefficient'})
    plt.title('Metric Correlation Matrix')
    
    plt.tight_layout()
    plt.savefig('metric_correlation_comprehensive.png', dpi=300, bbox_inches='tight')
    plt.close()
    
    print("Saved comprehensive correlation analysis: metric_correlation_comprehensive.png")

# Update the main execution section as well
if __name__ == "__main__":
    ...
    try:
        ...
        logfile.close()
        print("Multi-metric analysis complete! Check these files:")
        print("  - 'libero_multi_metric_comprehensive_analysis.txt' (analysis log)")
        print("  - 'dtw_analysis_detailed.png' (DTW-specific analysis)")
        print("  - 'ot_analysis_detailed.png' (Optimal Transport analysis)")
        print("  - 'cosine_analysis_detailed.png' (Cosine Similarity analysis)")
        print("  - 'metric_correlation_comprehensive.png' (Cross-metric correlation)")

# # Main execution
# if __name__ == "__main__":
#     tfrecord_path = '/scratch/work/zhangy50/RL/datasets/modified_libero_rlds/libero_goal_no_noops/1.0.0/libero_goal-train.tfrecord-00000-of-00016'
